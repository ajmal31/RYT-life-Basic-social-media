...
describe("POST /api/v1/users/login", () => {
  ...
  it("should return an error if email is not registered", async () => {
    ...
    const response = await request.post("/api/v1/users/login").send(nonExistentUser);
  ...
  it("should return an error if email or password is missing", async () => {
    ...
    const response = await request.post("/api/v1/users/login").send(incompleteLoginData);
  ...
  it("should return an error for invalid email format", async () => {
    ...
    const response = await request.post("/api/v1/users/login").send(invalidEmailLogin);
